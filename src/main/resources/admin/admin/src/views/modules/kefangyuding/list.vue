<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('kefangyuding','入住安排') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="ruzhuanpaiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已安排入住')">{{ contents.tableBtnFont == 1?'入住安排':'' }}</el-button>
                <el-button v-if="isAuth('kefangyuding','入住安排') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="ruzhuanpaiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已安排入住')">{{ contents.tableBtnFont == 1?'入住安排':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('kefangyuding','入住安排') && contents.tableBtnIcon == 0" type="success" size="mini" @click="ruzhuanpaiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已安排入住')">{{ contents.tableBtnFont == 1?'入住安排':'' }}</el-button>
    ...
    <ruzhuanpai-cross-add-or-update v-if="ruzhuanpaiCrossAddOrUpdateFlag" :parent="this" ref="ruzhuanpaiCrossaddOrUpdate"></ruzhuanpai-cross-add-or-update>
...
import ruzhuanpaiCrossAddOrUpdate from "../ruzhuanpai/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      ruzhuanpaiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    ruzhuanpaiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.ruzhuanpaiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.ruzhuanpaiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.ruzhuanpaiCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
