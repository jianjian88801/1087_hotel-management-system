<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="服务热线" prop="fuwurexian">
          <el-input v-model="ruleForm.fuwurexian" 
              placeholder="服务热线" clearable  :readonly="ro.fuwurexian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="服务热线" prop="fuwurexian">
              <el-input v-model="ruleForm.fuwurexian" 
                placeholder="服务热线" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiudianjianjie/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiudianjianjieCrossAddOrUpdateFlag = false;
						 this.parent.search();
