      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="预定编号" prop="yudingbianhao">
          <el-input v-model="ruleForm.yudingbianhao" 
              placeholder="预定编号" clearable  :readonly="ro.yudingbianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="预定编号" prop="yudingbianhao">
              <el-input v-model="ruleForm.yudingbianhao" 
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ruzhuanpai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ruzhuanpaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
